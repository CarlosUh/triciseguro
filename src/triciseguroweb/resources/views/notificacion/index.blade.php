        ...
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="card">
                    ...
                    <div class="card-body">
                        <a href="{{ url('/notificacion/create') }}" class="btn btn-success btn-sm" title="Add New Notificacion">
                            <i class="fa fa-plus" aria-hidden="true"></i> Add New
                        </a>

                        <form method="GET" action="{{ url('/notificacion') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>Tipo</th><th>Mensaje</th><th>Origen</th><th>Destino</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($notificacion as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->tipo }}</td><td>{{ $item->mensaje }}</td><td>{{ $item->origen_id }}</td><td>{{ $item->destino_id }}</td>
                                        <td>
                                            ...
                                            <form method="POST" action="{{ url('/notificacion' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                            ...
                            <div class="pagination-wrapper"> {!! $notificacion->appends(['search' => Request::get('search')])->render() !!} </div>
                        </div>
