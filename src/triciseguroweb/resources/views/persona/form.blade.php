...
<div class="form-group {{ $errors->has('ubicacion_id') ? 'has-error' : ''}}">
    <label for="ubicacion_id" class="control-label">{{ 'Ubicacion Id' }}</label>
    <input class="form-control" name="ubicacion_id" type="number" id="ubicacion_id" value="{{ isset($persona->ubicacion_id) ? $persona->ubicacion_id : ''}}" required>
    {!! $errors->first('ubicacion_id', '<p class="help-block">:message</p>') !!}
</div>
